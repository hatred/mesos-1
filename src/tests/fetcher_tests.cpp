...
TEST_F(FetcherTest, InvalidUser)
{
  ...
  CommandInfo commandInfo;
  commandInfo.set_user("b_o_g_u_s_m_a_x_i_m_u_s");
  ...
  // See FetcherProcess::chownDirectory().
  EXPECT_TRUE(fetch.failure().find("chown") != string::npos);
...
TEST_F(FetcherTest, NonExistingFile)
{
  ...
  // See FetcherProcess::___fetch().
  EXPECT_TRUE(fetch.failure().find("Failed to fetch") != string::npos);
}
...
TEST_F(FetcherTest, MalformedURI)
{
  ...
  EXPECT_TRUE(fetch.failure().find("Malformed") != string::npos);
}

...
public:
  ...
  static Try<Nothing> recoverCache(const SlaveID& slaveId, const Flags& flags);
...
{
public:
  FetcherProcess() : ProcessBase(process::ID::generate("__fetcher__")) {}
  ...
  virtual Try<process::Subprocess> run(
  ...
  // For testing only. This gets called when we are about to return from
  // makeCacheItems. If we wait for every concurrent task to get there,
  // we know that these tasks are contending on fetching (the same) URIs.
  virtual void contentionBarrier();
  ...
  public:
    ...
    public:
      ...
      // Indicates whether this file's download into the cache
      // has completed.
    ...
    void setSpace(Bytes bytes);

    void claimSpace(Bytes bytes);
    void releaseSpace(Bytes bytes);
    ...
    // Creates a new entry and inserts it into the cache table.
    memory::shared_ptr<Entry> createEntry(
        const std::string& cacheDirectory,
        const Option<std::string>& user,
        const CommandInfo::URI& uri);

    // Retrieves the cache entry indexed by the parameters.
    Option<memory::shared_ptr<Entry>> getEntry(
        const Option<std::string>& user,
        const std::string& uri);

    bool containsEntry(const Option<std::string>& user, const std::string& uri);

    // Removes an entry from the cache table without deleting the
    // associated cache file in the cache directory.
    void removeEntry(memory::shared_ptr<Entry> entry);
    ...
    // available.
    Try<std::list<memory::shared_ptr<Cache::Entry>>>
  ...
  private:
    ...
    // entries.
    hashmap<std::string, memory::shared_ptr<Entry>> table;
  };

  // Completely deletes a cache entry and its file. Warns on failure.
  // Public and virtual for testing.
  virtual Try<Nothing> deleteCacheEntry(
    const memory::shared_ptr<Cache::Entry>& entry);

  // Returns the number of cache files on disk for the given slave
  // (for all users combined). If requested, which is the default, also
  // checks if this number matches the number of registered cache entries.
  // For testing.
  size_t countCacheFiles(
      const SlaveID& slaveId,
      const Flags& flags,
      const bool checkEntries = true);

  // Returns the number of cache entries for the given slave (for all
  // users combined). For testing.
  size_t countCacheEntries(const SlaveID& slaveId, const Flags& flags);
...
private:
  // Creates the arguments for the external mesos-fetcher process and
  // invokes it.
  process::Future<Nothing> _fetch(
      const ContainerID& containerId,
      const std::list<FetcherInfo::Item>& nonCacheItems,
      const process::Future<std::list<FetcherInfo::Item>>& cacheItems,
      const std::string& sandboxDirectory,
      const std::string& cacheDirectory,
      const Option<std::string>& user,
      const Flags& flags);

  // Records successful and unsuccessful downloads to the cache and
  // reports the outcome of the mesos-fetcher run.
  ...
  std::list<process::Future<FetcherInfo::Item>> makeCacheItems(
      const std::list<CommandInfo::URI>& uris,
      memory::shared_ptr<std::list<memory::shared_ptr<Cache::Entry>>>
        referencedEntries,
      memory::shared_ptr<std::list<memory::shared_ptr<Cache::Entry>>>
        newEntries,
      const std::string& cacheDirectory,
  ...
  // Ensures (as needed by eviction) that there is enough space in
  // the cache to hold the result of downloading the given URI.
  // Then creates a fetcher action item for this URI with the
  // selected action value.
  process::Future<FetcherInfo::Item> downloadAndCache(
      const CommandInfo::URI& uri,
      const memory::shared_ptr<Cache::Entry>& entry,
      const std::string& cacheDirectory,
      const Flags& flags);

  // Ensures that there is enough space in the cache to download the
  // the file indicated by the given entry. Evicts other files as
  // necessary to make it so.
  process::Future<Nothing> reserveCacheSpace(
    const Try<Bytes>& requestedSpace,
    const memory::shared_ptr<Cache::Entry>& entry,
    const std::string& cacheDirectory);

  // Deletes cache entries with unsuccessful downloads.
  void cleanupNewCacheEntries(
      const memory::shared_ptr<std::list<memory::shared_ptr<Cache::Entry>>>&
            newEntries);
  ...
  void adjustCacheSpace(const memory::shared_ptr<Cache::Entry>& entry);
